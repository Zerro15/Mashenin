...
function uniqueEmail() {
  return `sql-smoke.${Date.now()}.${Math.random().toString(36).slice(2, 8)}@example.com`;
}
...
test('SQL teams: create -> list -> send message -> read', async () => {
  ...
  process.env.POSTGRES_USER = 'mashenin';
  process.env.POSTGRES_PASSWORD = 'mashenin_dev_pass';
  process.env.POSTGRES_DB = 'mashenin';
  ...
  try {
    ...
    const createRes = await app.inject({
      ...
      headers: auth(token),
      payload: { name: 'SQL Team', topic: 'testing sql mode' }
    });
    assert.equal(createRes.statusCode, 200);
    const team = createRes.json().team;
    assert.equal(team.name, 'SQL Team');
    ...
    await app.close();
    const { closePool } = await import('../src/lib/sql.js');
    await closePool();
  }
});
...
test('SQL rooms: send -> edit -> delete message', async () => {
  ...
  try {
    ...
    // Edit non-existent message (use valid UUID format)
    ...
    // Delete as non-author (create second user)
